...
def ThreScan(filepath, charge, Matrix):
    fp = open(filepath, 'rb')
    s = bitstring.ConstBitStream(filename=filepath)
    a = s.read('bin')
...
def main():
    ...
    filepath = [[] for i in range(5)]
    iTHR = np.arange(50, 91, 10)
    for threnum in range(5):
        for charge in range(lower, upper+1):
            print('iTHR=', iTHR[threnum], ' charge=', charge)
            ftdcontrol.SetThre(D3XX, 0x10, iTHR[threnum])
            filepath[threnum].append(
                Pulse(D3XX, 0x10, 'AnalogueScan', charge, iTHR[threnum]))

    ftdcontrol.ClosePipe(D3XX)

    for threnum in range(5):
        Matrix = np.zeros((512, 1024, 51), dtype=int)
        for num in range(0, len(filepath)):
            print('now processing threnum:', threnum, ' data num:', num)
            Matrix = ThreScan(filepath[threnum][num], num+lower, Matrix)
        ...
        sio.savemat('./ftd3xx/alpide/scanresult/iTHR0x%2x.mat' %
                    iTHR[threnum], {'Threshold': A, 'Noise': B})
        ...
        plt.savefig(
            './ftd3xx/alpide/scanresult/ThresholdDistribution_iTHR0x%2x.png' % iTHR[threnum])
        ...
        plt.ylabel('a.u.', horizontalalignment='right', y=1.0)
        plt.savefig('./ftd3xx/alpide/scanresult/NoiseDistribution_iTHR0x%2x.png' %
                    iTHR[threnum])
